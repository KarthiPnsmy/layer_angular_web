...
sampleControllers.controller('chatCtrl', function ($scope, $route, $location) {
  ...
  $scope.getConversationAvatars = function(conversationObject) {
    return conversationObject.participants.map(function(userId) {
      return window.layerSample.getDisplayName(userId).substr(0, 2).toUpperCase();
    });
  };
  ...
  $scope.getConversationTitle = function(conversationObject) {
    if (conversationObject) {
        ...
        //console.log("conversationObject.metadata.title = "+conversationObject.metadata.title);
      ...
      // A join of all participants names is the backup title.
        var result = conversationObject.participants.map(function(userId) {
            return window.layerSample.getDisplayName(userId);
        }).join(', ');
        console.log("result = "+result);
        return result;
      //return conversationObject.participants.join(', '); window.layerSample.users
    }
    return '';
  };
});
